      ...
      included do
        ...
        def run_catch_all(err:)
          ...
          if err.class == Stealth::Errors::UnrecognizedMessage
            Stealth::Logger.l(
              topic: 'catch_all',
              message: "[Level #{error_level}] for session #{current_session_id} #{err.message}"
            )
          else
            Stealth::Logger.l(
              topic: 'catch_all',
              message: "[Level #{error_level}] for session #{current_session_id} #{[err.class, err.message, err.backtrace.join("\n")].join("\n")}"
            )
          end
          ...
          if current_session.flow_string == 'catch_all'
            Stealth::Logger.l(
              topic: 'catch_all',
              message: "CatchAll triggered for session #{current_session_id} from within CatchAll; ignoring."
            )
            return false
          end
          ...
          if defined?(CatchAllsController) && FlowMap.flow_spec[:catch_all].present?
            ...
            if FlowMap.flow_spec[:catch_all].states.keys.include?(catch_all_state.to_sym)
              ...
              Stealth::Logger.l(
                topic: 'catch_all', message: "Stopping; we\'ve exceeded the number of defined catch_all states for session #{current_session_id}.")
              return false
            end
          end
        end
